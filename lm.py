...
class LM(object):
        ...
        for pred_index, prediction_id in enumerate(prediction_ids):
            ...
            if decoder_input_ids is not None:
                if isinstance(decoder_input_ids, tuple):
                    # Assuming the tuple contains only one element (a torch.Tensor)
                    decoder_input_ids = decoder_input_ids[0]
                assert len(decoder_input_ids.size()) == 2  # will break otherwise
                decoder_input_ids = torch.cat( [decoder_input_ids, torch.tensor([[prediction_id]], device=decoder_input_ids.device)],dim=-1)
            else:
    ...
    #             return self.params
    #         else:
    #             print("Parmas is empty call the predict token function first")
    ...
    def predict_token(self, inputs, topk=50, temperature=1.0):
        ...
        """.format(viz_id, viz_id, json.dumps(self.params))
        d.display(d.Javascript(js))
