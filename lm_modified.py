...
class LM(object):
    ...
    def generate(self, input_str: str,
        ...
        #     if version.parse(transformers.__version__) >= version.parse('4.13'):
        #         decoder_input_ids = self.model._prepare_decoder_input_ids_for_generation(input_ids[0].shape[0], None, None)
        #     else:
        #         decoder_input_ids = self.model._prepare_decoder_input_ids_for_generation(input_ids[0], None, None)
        # else:
        #     decoder_input_ids = None
        ...
        for pred_index, prediction_id in enumerate(prediction_ids):
            ...
            if decoder_input_ids is not None:
                # print(type(decoder_input_ids))
                assert len(decoder_input_ids[0].size()) == 2 # will break otherwise
                decoder_input_ids, device = decoder_input_ids
                decoder_input_ids = torch.cat(
                    [decoder_input_ids, torch.tensor([[prediction_id]], device=device)],
                    dim=-1
                )
            else:
                input_ids = torch.cat(
    ...
    def __call__(self, input_tokens: torch.Tensor):
        ...
        return OutputSeq(**{'tokenizer': self.tokenizer,
                            ...
                            'model_type': self.model_type,
                            'device': self.device,
                            'config': self.model_config}), self.activations[layer_type]
            ...
            if re.search(self.collect_activations_layer_name_sig, name):
                ...
                if self.collect_activations_flag:
                    self._hooks[name] = module.register_forward_hook(
                        lambda self_, input_, output, name=name: self._get_activations_hook(name, input_))
...
def _one_hot_batched(token_ids, vocab_size):
    if isinstance(token_ids, torch.Tensor):
        batch_size, num_tokens = token_ids.shape
    elif isinstance(token_ids, tuple) and len(token_ids) == 2:
        token_ids, device = token_ids
        batch_size, num_tokens = token_ids.shape
    else:
        raise ValueError("token_ids should be a torch.Tensor or a tuple of (torch.Tensor, device)")
